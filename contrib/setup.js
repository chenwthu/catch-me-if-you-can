$(function() {
    ...
    var setup = function() {
        ...
        invMap = getMap($('#terrain-layer'), gridShape);
    ...
    $('input[name=algorithm]').on('change', function() {
        chaser.algorithm = $('input[name=algorithm]:checked').val();
    });
    ...
    $('#catch-me').on('click', function() {
        var escape = function() {
            ...
            var target = [
                {x: Math.floor(map[0].length/2), y: Math.floor(map.length/2)},
                {x: 0, y: 0},
                {x: 0, y: map.length-1},
                {x: map[0].length-1, y: 0},
                {x: map[0].length-1, y: map.length-1}
            ];
            var k = 0;
            $.each(target, function(id, val) {
                if (Math.min(Math.abs(val.x-src.x), Math.abs(val.y-src.y)) > 10)
                    if ((val.x-target[k].x)*(dst.x-src.x) + (val.y-target[k].y)*(dst.y-src.y) > 0)
                        k = id;
            });
        ...
        var chase = function() {
            ...
            var track = chaser.search(gridShape, map, src, dst);
            track.shift();
            $.each(track, function(id, val) {
                trackPainter.paint(gridShape, 'fill', val.x, val.y, '#f05654');
            });

            if (track.length>0 && track[0].x==dst.x && track[0].y==dst.y) {
                catching = 0;
                caught = 1;
