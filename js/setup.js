$(function() {
    ...
    // whether animation effect is enabled
    var animation = 1; // 1 for enabled, 0 for disabled
    var animationTimer; // timer for animation effect

    /*
     *  mouse event on map
     *  Click (and drag) to put/remove obstacles. Click and drag to move
     *  src and dst.
     */
    var dragging = -1;
    ...
    .on('mousedown mousemove', function(e) {
        ...
        e = e || window.event;
        var button = ('buttons' in e && e.buttons!==undefined) ? e.buttons
            : (('which' in e && e.which!==undefined) ? e.which : e.button);
    ...
    $('input[name=grid-shape]').on('change', function() {
        if (animationTimer) $('#catch-me').click();
    ...
    $('#animation').on('click', function() {
        animation ^= 1;
        $('#animation').html(animation ? 'Disable Animation' : 'Enable Animation');
    });

    $('#catch-me').on('click', function() {
        var startAnimation = function() {
            $('#catch-me').html('STOP!');
            $('#animation,#clear-obstacles,#clear-tracks').hide();
        };

        var stopAnimation = function() {
            animationTimer = clearInterval(animationTimer);
            $('#catch-me').html('CATCH ME!');
            $('#animation,#clear-obstacles,#clear-tracks').show();
        };
        ...
        var showTrack = function() {
            ...
            if (result.track.length == 0) { // no solution
                var ctx = $('#track-layer')[0].getContext('2d');
                ctx.font = 'italic ' + gridShape.a*3 + 'px Arial';
                ctx.fillStyle = '#f05654';
                ctx.fillText('No solution!',
                    $('#map-container').scrollLeft() + gridShape.a,
                    $('#map-container').scrollTop() + gridShape.a*4);
            }
        };
